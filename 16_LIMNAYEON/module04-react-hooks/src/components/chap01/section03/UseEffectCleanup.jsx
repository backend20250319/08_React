...
function CleanupComponent() {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      // timer에는 생성된 setInterval객체 값을 담아둔다.
      setCount((prev) => prev + 1);
      console.log(`타이머 실행 중...`);
    }, 1000);

    /*
      useEffect 내부에서 함수를 반환하면, 컴포넌트가 언마운트 될 때 호출된다.
      이는 주로 리소스 해제, 정리 작업 등을 수행하는 용도로 사용한다. (unmount 단계)
    */
    return () => {
      clearInterval(timer);
  ...
  return (
    <div style={{ border: "1px solid black", padding: "10px" }}>
      <h2>CleanComponent</h2>
      <p>현재 카운트: {count}</p>
    </div>
  );
}
export default function UseEffectCleanu() {
